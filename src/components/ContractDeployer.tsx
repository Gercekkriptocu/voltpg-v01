...
const ContractDeployerComponent = function ContractDeployer({ onLog }: ContractDeployerProps): ReactElement {
  ...
  const deployContract = async (customParams?: {
    ...
    try {
      ...
      if (!window.ethereum) {
        const errorMsg = 'MetaMask or another Ethereum wallet is required. Please install MetaMask extension.'
      ...
      onLog('✅ MetaMask detected', 'success')

      // 5️⃣ Wallet hesaplarını al - MetaMask bağlantı penceresi açar
      console.log('📱 Requesting wallet accounts...')
      onLog('📱 Opening MetaMask connection...', 'info')
      ...
      while (!receipt && attempts < maxAttempts) {
        ...
        try {
          // RPC'ye receipt sorgusu yap - MANUEL FETCH with error handling!
          ...
          // Check if response is OK (not 500, 502, etc.)
          if (!response.ok) {
            console.warn(`⚠️ RPC returned status ${response.status}, retrying...`)
            attempts++
          ...
          if (data.error) {
            console.warn(`⚠️ RPC error: ${data.error.message}, retrying...`)
            attempts++
            continue
          }
          ...
          console.warn(`⏳ Fetch error (attempt ${attempts + 1}/${maxAttempts}):`, e.message || e)
          // Continue to next attempt even if fetch fails
        }
      ...
      if (errorMessage.toLowerCase().includes('user rejected') || errorMessage.toLowerCase().includes('denied')) {
        ...
        onLog('💡 Please approve the transaction in MetaMask to continue.', 'info')
        ...
        if (currentNetwork.chainId === 91342) {
          ...
          onLog('🔗 Stable Faucet: https://faucet.testnet.stable.xyz', 'info')
        }
      ...
      } else if (errorMessage.toLowerCase().includes('no accounts')) {
        ...
        onLog(`💡 Error details: ${errorMessage.slice(0, 100)}`, 'info')
      }
    } finally {
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
